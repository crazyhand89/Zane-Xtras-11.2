...
public class SkeletonBow extends ItemBow {
	
	public SkeletonBow() {
		this.maxStackSize = 1;
		this.setMaxDamage(4123);
		this.setFull3D();
		this.setUnlocalizedName(ModInfo.MODID + "_skeletonbow");
		this.setRegistryName(ModInfo.MODID + "_skeletonbow");
				...
				new IItemPropertyGetter() {
					...
					public float apply(ItemStack stack, @Nullable World worldIn,
						...
						return entityIn != null && entityIn.isHandActive()
								&& entityIn.getActiveItemStack() == stack ? 1.0F
										: 0.0F;
					}
				});
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!itemstack.isEmpty() || flag) {
				if (itemstack.isEmpty()) {
					itemstack = new ItemStack(ZaneItems.goldenArrow);
				}
				...
				if (f >= 0.1D) {
					boolean flag1 = entityplayer.capabilities.isCreativeMode
							|| (itemstack.getItem() instanceof GoldenArrow
									&& ((GoldenArrow) itemstack.getItem())
											.isInfinite(itemstack, stack,
													entityplayer));
					
					if (!worldIn.isRemote) {
						GoldenArrow GoldenArrow = ((GoldenArrow) (itemstack
								.getItem() instanceof GoldenArrow
										? itemstack.getItem()
										: ZaneItems.goldenArrow));
						ItemArrow itemarrow = ((ItemArrow) (itemstack
								.getItem() instanceof ItemArrow
										? itemstack.getItem() : Items.ARROW));
						EntityArrow entityarrow = itemarrow.createArrow(worldIn,
						...
						int j = EnchantmentHelper
								.getEnchantmentLevel(Enchantments.POWER, stack);
						
						if (j > 0) {
							entityarrow.setDamage(
									entityarrow.getDamage() + j * 0.5D + 0.5D);
						}
						
						int k = EnchantmentHelper
								.getEnchantmentLevel(Enchantments.PUNCH, stack);
	...
	private float updatePullProperty(ItemStack stack, World worldIn,
	...
	protected boolean isArrow(ItemStack stack) {
		return stack.getItem() instanceof GoldenArrow;
	}
}
